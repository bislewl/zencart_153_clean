...
$requiredBinaries = array('/usr/local/bin/git', 'rsync');
...
if (!is_dir(TMP_DIR)) {
	...
	$commands[] = sprintf(
		'/usr/local/bin/git clone --depth=1 --branch %s %s %s'
		, BRANCH
		, REMOTE_REPOSITORY
		, TMP_DIR
	);
	...
	$commands[] = sprintf(
		'/usr/local/bin/git --git-dir="%s.git" --work-tree="%s" fetch origin %s'
		, TMP_DIR
		, TMP_DIR
		, BRANCH
	);
	$commands[] = sprintf(
		'/usr/local/bin/git --git-dir="%s.git" --work-tree="%s" reset --hard FETCH_HEAD'
		, TMP_DIR
		, TMP_DIR
	);
}
...
if (defined('VERSION_FILE') && VERSION_FILE !== '') {
	$commands[] = sprintf(
		'/usr/local/bin/git --git-dir="%s.git" --work-tree="%s" describe --always > %s'
		, TMP_DIR
		, TMP_DIR
		, VERSION_FILE
	);
